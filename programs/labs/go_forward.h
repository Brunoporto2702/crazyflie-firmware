...
int main() {
  ...
  tim.start();
  float t = 0;
  while (t < 20.0 && (att_est.p < 2 * 3.14) && (att_est.q < 3.14 * 4) &&
    ...
    if (flag) {
      ...
      att_est.estimate();
      ver_est.predict();
      hor_est.correct(ver_est.z, att_est.theta, att_est.phi, att_est.q,
                      att_est.p);

      hor_cont.control(hor_est.x, hor_est.y, x_r, y_r, hor_est.v_x, hor_est.v_y);
